...
class CyodaRepository(CrudRepository):
    ...
    def find_all(self, meta) -> List[Any]:
        return self.get_all_entities(meta)
    ...
    def update(self, meta, id, entity: Any) -> Any:
        ...
        if entity is None:
            return self.launch_transition(meta)
    ...
    @staticmethod
    def model_exists(token, entity_name, version) -> bool:
    ...
    @staticmethod
    def get_model(token, entity_name, version):
    ...
    def get_all_entities(self, meta):
        path = f"entity/TREE/{meta["entity_model"]}/{meta["entity_version"]}"
        response = send_get_request(meta["token"], API_URL, path=path)
        logger.info(response.json())
        return response.json()

    def launch_transition(self, meta):
